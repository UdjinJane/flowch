...
class FluxLoraCoreV02:
    ...
    def init_transformer_with_lora():
        ...
        def _фильтр_магистрали(модуль, имя_модуля):
            # Квантуем линейные слои строго по белому списку мишеней Хромы
            if isinstance(модуль, nn.Linear):
                # Проверяем, что слой принадлежит тяжелым блокам обработки мантиссы
                _целевой_слой = any(_блок in имя_модуля for _блок in ["double_blocks", "single_blocks"])
                if _целевой_слой:
                    # Изолируем микро-модуляции внутри блоков от сжатия
