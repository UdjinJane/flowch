    ...
    for epoch in range(1, TrainConfig.NUM_EPOCHS + 1):
        ...
        for step, mega_batch in enumerate(dataloader):
            ...
            for frame_idx in range(total_frames):
                ...
                # Честный расчет img_ids по физической форме входящих латентов Хромы
                _, _, h_latent, w_latent = latents.shape  # Нативно вытаскиваем 128x128
                h_patches = h_latent // 2  # 128 // 2 = 64 патча
                w_patches = w_latent // 2  # 128 // 2 = 64 патча
                ...
                # Нативный скаляр guidance, расширенный под батч B=1 строго по chroma_model.py кузнецов
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
                    pred_tensor = checkpoint(
                        ...
                        t_model_scale,
                        prompt_embeds,       # 6-й позиционный аргумент: txt-эмбеддинги
                        guidance_tensor,     # 7-й позиционный аргумент: timesteps/guidance шлюз кузнецов
