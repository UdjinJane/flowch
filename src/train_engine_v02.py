# Финальная версия train_engine_v02.py с исправлениями:
import os
import sys
import gc
import shutil
...
def main_train_loop():
    ...
    if len(trainable_params) == 0:
        raise RuntimeError("[КРИТИЧЕСКИЙ ОТКАЗ] ЛОРА АДАПТЕР ПОЛНОСТЬЮ ОБЕЗГЛАВЛЕН! Градиенты заблокированы!")
    ...
    while run_reactor:
        ...
        for batch in dataloader:
            ...
            if torch.isnan(timesteps_attr).any() or torch.isinf(timesteps_attr).any():
                raise RuntimeError("[КРИТИЧЕСКИЙ ОТКАЗ] Invalid timestep values!")
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                raise RuntimeError(f"[КРИТИЧЕСКИЙ ОТКАЗ] Loss exploded: {loss.item()}")
